...
async function createPDF(order: FullOderType) {
  ...
  // Create second copy
  await createReceiptCopy(pdf, order, 148.5);
...
async function createReceiptCopy(
  ...
  pdf.text(
    "Las prendas son artículos nuevos. No aplican garantías. El costo de envío",
    34,
    termsY + 10
  );
  pdf.text(
...
function addInvoiceDetails(pdf: jsPDF, order: FullOderType, yOffset: number) {
  pdf.setFontSize(16);
  pdf.setTextColor(COLORS.text[0], COLORS.text[1], COLORS.text[2]);
  pdf.text(`R#${order.orderNo}`, 185, yOffset + 8, { align: "right" });
  pdf.setFontSize(11);
