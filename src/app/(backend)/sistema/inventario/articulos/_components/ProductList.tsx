...
export function ProductList({ items }: { items: ProductType[] }) {
...

      {
        ...
        cell: ({ row }) => (
          <Image
            ...
            width={100}
            height={100}
            className="capitalize text-xs min-w-10 h-auto "
          />
        ),
      },
        ...
        cell: ({ row }) => {
          const ActionCell = () => {
            ...
            const viewOrder = React.useCallback(async () => {
              router.push(`/sistema/inventario/articulos/${row.original.id}`);
            }, []);
            return (
              <DropdownMenu>
