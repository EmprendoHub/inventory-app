...
export function ExpenseList({ expenses }: { expenses: ExpenseType[] }) {
  ...
  const columns = React.useMemo<ColumnDef<ExpenseType>[]>(
    () => [
      {
        accessorKey: "type",
        header: ({ column }) => (
        ...
        cell: ({ row }) => (
          <div className="lowercase text-xs w-20">{row.getValue("type")}</div>
        ),
      },
      {
        accessorKey: "amount",
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const ActionCell = () => {
            ...
            const viewExpense = React.useCallback(async () => {
              router.push(
                `/sistema/contabilidad/gastos/editar/${row.original.id}`
              );
            }, []);
            return (
              <DropdownMenu>
  ...
  const table = useReactTable<ExpenseType>({
    data: expenses,
    columns,
