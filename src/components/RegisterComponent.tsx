...
const RegisterFormComponent = ({ cookie }: { cookie: any }) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (email === "") {
      console.log("Por favor agregue su correo electrónico para registrarse.");
      return;
    }

    if (!isValidEmail(email)) {
      console.log("Utilice un correo electrónico válido.");
      return;
    }

    if (!isValidPhone(phone)) {
      console.log("Utilice un teléfono válido.");
      return;
    }
    ...
    executeRecaptcha("enquiryFormSubmit").then(async (gReCaptchaToken) => {
      try {
        ...
        if (res.status === 400) {
          console.log("Este correo electrónico y/o el teléfono ya esta en uso");
          setError("Este correo electrónico y/o el teléfono ya esta en uso");
        }
        if (res.ok) {
          console.log("Se registró exitosamente al usuario");
          signIn();
          return;
        }
      } catch (error) {
